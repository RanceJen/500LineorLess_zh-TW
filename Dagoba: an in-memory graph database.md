...
在很久以前，當這個世界還很年輕，所有的資料都在單一的文件中快樂地行走，如果你想要你的資料們跳過藩籬，你只需要在它們的路徑上將藩籬設得低一些，那每個資料就可以依次跳過它。放入[打孔卡](https://zh.wikipedia.org/wiki/打孔卡)，拿出打孔卡。生活很簡單，編程也很輕鬆。
...
接著發生了隨機訪問革命，數據在山坡上自由放牧。放牧數據成為一個嚴重問題：如果您可以隨時訪問任何數據，你如何知道接下來要選擇哪一個？藉著形成項目[^項目]間的連結和透過它們的鏈接集合將單元群編隊，把資料趕進圍欄。詢問數據就意味著挑選一隻綿羊並拉動與之相連接的所有東西。
...
後來的程序員背離了這一項傳統，對於要怎麼聚合數據實施了一系列的規則[^關係理論]。它們不是直接將不同的數據捆綁在一起，而是按內容組成群集，將數據分解為一口大小的片段，收集在筆中並使用名稱標籤進行整理。問題是以聲明方式提出的，導致部分分解的數據（一個關係主義者稱之為“正常”的狀態）累積到返回給程式設計師的 frankencollection 中。

> frankencollection 待確認意思
...
在大部分記錄的歷史中，這種關係模型占據了至高無上的地位。經歷了兩次主要語言戰和無數小規模衝突，它的統治地位變得無法被挑戰。它提供了你在模型中可以要求的一切，但要忍受低效率，笨拙和缺乏可擴展性的微小代價。對遠古時代來說，那是程序員願意支付的價格。然後網際網路發生了。
...
分散式革命再次改變了一切。數據突破了空間的限制，並且在機器之間漫遊。使用 CAP 的理論家破壞了關係壟斷，為新的牧業技術打開了大門......。其中一些可以追溯到最早馴化隨機訪問數據的嘗試。我們將看看其中一種，稱為圖形數據庫。

> [CAP 理論](https://en.wikipedia.org/wiki/CAP_theorem)
...
[^項目]：最初的數據庫設計之一是階層式模型，它將項目分組為樹狀階層式結構，並仍然被用在 IBM IMS 產品（高速交易處理系統）的基礎。它的影響也可以在 XML，文件系統和地理信息存儲中看到。這個由 Charles Bachmann 發明並由 CODASYL 標準化的網路模型透過允許多個父代、形成DAG 而不是樹，來一般化階層模型。 這些導航數據庫模型在 1960 年代開始流行，並持續佔據主導地位，直到性能提升的關係數據庫在 1980 年代開始被使用。

> [Hierarchical Model](https://wiki.mbalib.com/zh-tw/%E5%B1%82%E6%AC%A1%E6%A8%A1%E5%9E%8B)
...
在本章中，我們將建構一個圖形數據庫[^dagoba]。在建構它時，我們將會探索問題空間，為我們的設計決策生成多個解決方案，比較這些解決方案以了解它們之間的權衡，最後為我們的系統選擇正確的解決方案。程式碼緊湊度的優先順序會比一般來得高，但該過程將反映自遠古以來軟件專業人員使用的過程。本章的目的是教授這個過程，並建立一個圖數據庫[^目的]。
...
[^dagoba]：這個數據庫起源於管理有向無環圖或稱作 DAG 的函式庫。它的名字 “Dagoba” 本來打算最後帶一個沉默的 'h'，向佈滿沼澤的虛構星球致敬，但是有一天我們發現 sans-h 版本的巧克力棒背面是指一個用來默默地思考事物之間聯繫的地方，而這似乎更合適。

[^purpose]: The two purposes of this chapter are to teach this process, to build a graph database, and to have fun.

[^目的]本章節的兩個目的是教授這個過程、建立一個圖數據庫，而且玩得開心。
...
使用圖形數據庫將使我們能夠以優雅的方式解決一些有趣的問題。圖形是一種非常自然的數據結構，用於探索事物之間的聯繫。這個意義上的圖是一組頂點和一組邊，換句話說，它是一串由線連接的點。那數據庫呢？“數據庫”就像數據的堡壘。你將數據放入其中並從中獲取數據。
...
所以我們可以用圖形數據庫解決哪些問題呢？假設你喜歡追蹤祖先樹：父母，祖父母，表親的孫子女，之類的事情。你希望開發一個系統，讓你可以自然而優雅的查詢，例如：“第一次刪除後，誰是 Thor 堂兄弟的子女？” 或者 “Freyja 與 Valkyries 的聯繫是什麼？”

<譯者註> [cousins twice removed](http://raylinusa.blogspot.com/2007/08/first-cousin-once-removed-first-cousin.html)
...
此數據結構的合理模式是擁有實體表和關係表。對 Thor 父母的查詢可能看起來像
...
但我們如何將其擴展到祖父母？我們需要做一個子查詢，或者使用一些其他特定供應商的 SQL 擴充。當我們要取得表親的子女時，我們將需要*很多*的 SQL。
...
我們想寫什麼？既簡潔又靈活的東西；以自然的方式為我們的查詢建模型，並擴展到其他類似的查詢。 `second_cousins('Thor')` 很簡潔，但它沒有給我們任何彈性。上面的 SQL 很彈性，但缺乏簡潔性。
...
像 “Thor.parents.parents.parents.children.children.children” 這樣的東西達到了相當不錯的平衡。原始的表達方式讓我們可以靈活地提出許多類似的問題，但查詢簡潔且自然。這種特定的措辭給了我們太多的結果，因為它包括了第一代堂兄弟和兄弟姐妹，但我們在這裡要使用格式塔。
...
我們可以建構什麼最簡單的東西，來提供這種界面？我們可以創建頂點列表和邊緣列表，就像關係模式一樣，然後建構一些輔助函數。它可能看起來像這樣：
...
上述函數的本質是遍歷列表，為每個項目評估一些代碼並建構結果的累加器。然而，這可能沒有那麼清晰，因為循環結構引入了一些不必要的複雜性。
...
如果可以為此目的設計一個更具體的循環結構，那就太好了。實際上，`reduce` 函數就是這樣：給定一個列表和一個函數，它會評估列表中每個元素的函數，同時透過每個評估過程對累加器進行線程化。

寫成這個更實用的樣式，我們的查詢變得更短更清晰：

> TODO. check the usage of reduce
```javascript
...
給定一個頂點（vertices）列表，我們對邊緣（edges）做 reduce，如果邊緣的子頂點在我們的輸入列表中，則將邊緣的父頂點添加到累加器。 `children` 函數是相同的，不過是檢查邊的父頂點來決定是否加入該邊緣的子頂點。

這些函數是有效的 JavaScript，但使用了一些瀏覽器在撰寫本文時尚未實現的功能。這個翻譯版本今天可以使用：
...
它要用相反方向讀，且讓我們迷失在愚蠢的括號中，但在其他方面非常接近我們想要的。花一點時間查看代碼。你能看到改善它的方法嗎？
...
我們將邊緣視為全局變量，這意味著我們使用這些輔助函數時，一次只能擁有一個數據庫。這非常的侷限。
...
我們也根本沒有使用頂點。這告訴我們什麼？它意味著我們需要的所有東西都在邊緣陣列中，而在這種情況下是真的：頂點值是純量，所以它們在邊緣陣列中獨立存在。如果我們想回答諸如 “什麼是 Freyja 與 Valkyries 的聯繫？” 之類的問題。我們需要向頂點添加更多數據，這意味著使它們成為複合值，這意味著邊緣陣列應該引用頂點而不是複製它們的值。
...
對於我們的邊緣也是如此：它們包含 “in” 頂點和 “out” 頂點[^vertexnote]，但沒有優雅的方式來合併額外的信息。我們需要回答這些問題，比如 “Loki 有多少繼父母？” 或者 “在 Thor 出生之前，Odin 有多少個孩子？”
...
你不必認真地瞇著眼睛告訴我們兩個選擇器的代碼看起來非常相似，這表示它們可能會有更深層次的抽象。
...
[^vertexnote]：請注意，我們將邊緣建模為一對頂點。還有注意那些對是有序的，因為我們使用的是陣列。這意味著我們正在建一個*有向圖*的模型，其中每個邊緣都有一個起始頂點和一個結束頂點。我們的 “點和線” 視覺模型變成了 “點和箭頭” 模型。
  這增加了我們模型的複雜性，因為我們必須持續追蹤邊緣的方向，但它也允許我們提出更有趣的問題，比如 “哪個頂點指向頂點3？” 或 “哪個頂點有最外邊緣？” 如果我們需要對無向圖進行建模，我們可以為有向圖中的每個現有邊添加反轉邊。走向另一個方向可能很麻煩：用無向圖來模擬有向圖。你能想到方法嗎？

## Build a Better Graph（建造一個更好的圖）
...
讓我們解決一些我們已經發現的問題。我們的頂點和邊緣是全域的，讓我們有了一次只能一張圖的限制，但我們希望有更多。要解決這個問題，我們需要一些結構。讓我們從命名空間開始。
...
我們將使用一個物件作為我們的命名空間。JavaScript 中的物件大多只是一組無序的鍵/值對。我們在 JavaScript 中只有四種基本數據結構可供選擇，因此我們將大量使用這一結構。（在派對上向人們提問的一個有趣的問題是 “JavaScript 中的四種基本數據結構是什麼？” ）

> 譯者註：JavaScript 中的四種基本數據結構？

現在我們需要一些圖表。我們可以使用經典的 OOP 模式構建它們，但 JavaScript 為我們提供了原型繼承，這意味著我們可以建構一個原型物件，我們將其稱為 `Dagoba.G`，然後使用工廠函數實例化它的副本。這種方法的一個優點是我們可以從工廠返回不同類型的對象，而不是將創建過程綁定到單個類別建構函數。所以我們免費獲得一些額外的彈性。
...
我們將接收兩個可選參數：頂點列表和邊緣列表。 JavaScript 對參數相當鬆散，因此所有命名參數都是可選的，如果沒有提供，則預設為 `undefined`[^optionalparams]。在建構圖形並使用 V 和 E 參數之前，我們通常會有頂點和邊緣，不過在創建時沒有頂點和邊緣，並以編程方式建構圖形也很常見[^graphbuilding]。
...
[^optionalparams]：它在另一個方向上也是鬆散的：所有函數都是可變參數，並且所有參數都可以透過 `arguments` 物件獲得，這幾乎就像一個陣列但不完全相同。（“Variadic” 是一種花俏的方式來說明函數有不確定數量的參數。“函數有不確定數量的參數” 是一種花俏的方式來說明它需要可變數量的變量。）
...
[^graphbuilding]：這裡的 `Array.isArray` 檢查是為了區分我們的兩種不同使用情境，但一般來說，為了專注於架構而不是垃圾箱，我們不會做太多許多人對生產代碼期望的驗證。
...
然後我們創建一個新物件，它擁有我們原型的所有優點而沒有任何缺點。我們為邊緣創建了一個全新的陣列（其中一個基本的 JS 數據結構），另一個用於頂點，一個名為 `vertexIndex` 的新物件和一個 ID 計數器，後面的兩個會在之後談到更多。（想一想：為什麼我們不能把它們放在原型中？）

然後我們從工廠內部調用 `addVertices` 和 `addEdges`，現在讓我們來定義它們。
